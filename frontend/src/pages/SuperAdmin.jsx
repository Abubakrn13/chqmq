...
export default function SuperAdmin() {
  ...
  return (
    <div className="super">
      ...
      {detail && (
        <Modal title={`Batafsil · ${detail.shop.name}`} onClose={() => setDetail(null)} max="620px">
          <div className="detail-grid">
            <div><span>Yaratilgan</span><b>{fmtDate(new Date(detail.shop.createdAt))}</b></div>
            <div><span>Tushum</span><b>{money0(detail.metrics.revenue)}</b></div>
            <div><span>Foyda</span><b>{money0(detail.metrics.profit)}</b></div>
          ...
          <h4 style={{ marginTop: 16, marginBottom: 6 }}>Filiallar ({detail.branches.length})</h4>
          <div className="detail-list">
            {detail.branches.map((b) => <div key={b.id} className="dl-row"><b>{b.name}</b><span>{b.address || "—"}</span></div>)}
          </div>
          <h4 style={{ marginTop: 16, marginBottom: 6 }}>Xodimlar ({detail.users.length})</h4>
          <div className="detail-list">
            {detail.users.map((u) => (
              <div key={u.id} className="dl-row">
                <b>{u.name}</b>
                <span>{ROLES[u.role]} · {u.phone}</span>
              </div>
            ))}
          </div>
          <h4 style={{ marginTop: 16, marginBottom: 6 }}>So'nggi sotuvlar</h4>
          <div className="detail-list">
            {detail.recent.length ? detail.recent.map((s) => (
              <div key={s.id} className="dl-row">
                <b>{money0(s.total)}</b>
                <span>{s.userName} · {s.pay} · {fmtDate(new Date(s.ts))}</span>
              </div>
            )) : <div className="muted-xs">Sotuvlar yo'q</div>}
          </div>
          <div className="modal-foot">
            <button className="btn primary block" onClick={() => setDetail(null)}>Yopish</button>
          </div>
        </Modal>
      )}
