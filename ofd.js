...
/* One-time welcome message sent when a chat is first linked to a shop.
   Advertises the public channel and community group. Reads links from
   env so the owner can update them without a code change:
     TELEGRAM_CHANNEL_LINK  e.g. https://t.me/chaqmoq_uz
     TELEGRAM_GROUP_LINK    e.g. https://t.me/chaqmoq_uz_chat
   Returns true if Telegram accepted the message. */
async function sendWelcome(chatId) {
  const channel = process.env.TELEGRAM_CHANNEL_LINK || "https://t.me/chaqmoq_uz";
  const group = process.env.TELEGRAM_GROUP_LINK || "";
  const parts = [
    "<b>⚡ Chaqmoq'ga xush kelibsiz!</b>",
    "",
    "Endi bu chatga:",
    "• Har kuni ertalab 9:00 da — kunlik hisobot",
    "• Har kuni ertalab 9:05 da — to'liq zaxira fayli",
    "• Kam qolgan mahsulotlar uchun buyurtma tavsiyalari",
    "keladi.",
    "",
    `📣 Yangiliklar kanali: ${channel}`,
  ];
  if (group) parts.push(`💬 Muhokama guruhi: ${group}`);
  parts.push("", "<i>Muammo yoki taklif bo'lsa — kanal ostidan yozing.</i>");
  const r = await sendMessage(chatId, parts.join("\n"));
  return !!(r && r.ok);
}

// Compose the daily report text for a shop (yesterday's sales + low stock).
...
module.exports = { sendMessage, sendDocument, sendWelcome, reportFor, runDailyReports, runDailyBackups, startScheduler };
