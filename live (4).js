...
const { sendMessage, sendDocument, reportFor, sendWelcome } = require("../integrations/telegram");
...
router.post("/link", requireOwner, async (req, res) => {
  ...
  const row = {
    ...
    enabled: true,
    createdAt: Date.now(),
    welcomeSent: false, // set true after we deliver the intro message
  };
  db.update((d) => d.chatIds.push(row));

  // Fire off a one-time welcome with our channel & community links.
  // Only the first successful link gets it — subsequent links to the same
  // chat won't re-spam because we mark welcomeSent on success. We don't
  // block the response on this: the link should succeed even if the
  // welcome delivery fails (e.g. bot not started yet).
  sendWelcome(chatId).then((ok) => {
    if (ok) {
      db.update((d) => {
        const c = d.chatIds.find((x) => x.id === row.id);
        if (c) c.welcomeSent = true;
      });
    }
  }).catch(() => {});
